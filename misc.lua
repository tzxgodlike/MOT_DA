...
--- 6 获取真值
--- 7 获取检测值
--- 8 获取x,y状态的最大值，用于normlization data
  ...
  while true do
    ...
    for str in matchfunc do
      if key==nil then key = str    -- new line, set key
        ...
        else val = str -- else set as string
        end     
      end
--       print(key,val)
--       sleep()
...
--------------------------------------------------------------------------
--- Experimental: moves a torch tensor to gpu if needed.
--- Otherwise, converts it to float
...
--- 5 onNetwork()----------------------------------------------------------
function onNetwork()
  attr=lfs.attributes(getRNNTrackerRoot()..'graph/RNNBFForwardGraph.dot')
  -- print(attr)
...
--   F = math.min(tabLen(gt)-ts+1,F)
  local tL,minKey, maxKey = tabLen(gt)--Returns number of elements in a table, as well as min. and max. key
  local F = maxKey--number of frames in a batch
  local N = 255--max. number of targets / detections
  ...
  for t in pairs(gt) do
    ...
    for i = 1, N do
      ...
      local gtBBox = gt[t][i]--某一帧t中第i个目标数据，为1X2维的
      if gtBBox then 
        for dim = 1,nDim do
...
function getGT(seqName)
  local datafile = getDataDir() .. seqName .. "/gt.txt"
  local gt = 0
  local gtAvailable = 1
...
--     error("Error: GT file ".. datafile .." does not exist")
    print("WARNING: GT file ".. datafile .." does not exist",1)
    -- default zero GT
    gt = {}
...
function getDetections(seqName, detfile)
  detfile = detfile or getDataDir() .. seqName .. "/det.txt"
...
function getDetTensor(seqName, detfile)
  ...
  -- zh改 local imgHeight, imgWidth = getImgRes(seqName)
  imgWidth = 1
  local F = tabLen(detTab)--测量表中的帧数
  -- local nDim = 2   -- bounding box
  local nDim = 4  -- 改 bounding box
  ...
  for t=1,F do
    if detTab[t] then -- skip if no detections present in frame 
      ...
      --  detections[detID][t] = getFoot(detBBox)[1][1] / imgWidth  
          detections[{{detID},{t},{}}] = detBBox:narrow(2,1,nDim) / imgWidth--x,y,w,h除以图片宽度归一化
      end
    end
  end
...
--- 8 获取x,y状态的最大值，用于normlization data
function getStateMax(seqTable)
  if StateMax==nil  then StateMax={} end
  for _, seqName in pairs(seqTable) do
    if StateMax[seqName]==nil then 
      StateMax[seqName]={} 
      ---gt中状态的最大值---------
      ...
      -- print('max') print(max)
      ---将最大值赋值给StateMax---------
      StateMax[seqName]['MaxX'],StateMax[seqName]['MaxY']=max[1][1]+1,max[1][2]+1
      -- print(StateMax[seqName]['MaxX'])
      -- print(StateMax[seqName]['MaxY'])
      -- abort()
    end
  end
  return StateMax
...
function cleanDataTensor(data)
  local N,F,D = getDataSize(data)
  -- print('data') print(data[1])abort()
  -- local exTar torch.ne(torch.sum(data),0)
...
function getImgRes(seqName)
  ...
  local imFile = getDataDir() .. seqName .. "/det/000001-acf.jpg"
...
--   local timer = torch.Timer()
  ...
  -- print('hungarianL ass') print(ass)
...
--   local nFeat = 2 + 2*D + 0
  ...
  for s=1,tL do
    ...
    local tracks = trTable[s] --某个序列的数据 NxFxD
...
--     print(tracks)
    for i=1,N do --N个目标
      ...
      local exfr = torch.find(tr:squeeze()) -- 1D vec with frame nums，1，2，3，……，
      ...
      if trLength > 0 then
        local featTab = {}
	      -- start, end point
	      local s = exfr[1]--1
        local e = exfr[-1]--temp_win最大的帧数
        table.insert(featTab, s)--开始帧，一个整数
	      table.insert(featTab, e)--结束帧，
        ...
        -- 	print(m)
        -- 	print(featVec)
        m = m:cat(featVec,1)--在第1维度上联合 最终的维度为：(maxTarget+1 X nFeat)
      end
    end
--     print(tracks)
--     print(m)
    ...
    if m:size(1)>1 then
      m = m:sub(2,-1) -- remove first dummy row 去除第一行
--       print(m)
      mmean = torch.mean(m,1) --维数为：(1 X nFeat),返回每列的平均值，
      mstd = torch.std(m,1) --维数同上，返回每列的标准差
...
--  abort()
  return trackModels--维数：(序列数*2 X nFeat)
--   
end
...
-- TODO docs
--------------------------------------------
--- data:( maxTarget , T , nClasses)
--- return:lab
--- 1.HA=false,lab为最大概率值的索引
...
-------------------------
--- input: (mini_batch_size , maxT*stateDim ) t时刻的maxT个目标的状态
--- detections: (mini_batch_size * max_m , temp_win , D)
--- return allDistBatches:(mini_batch_xize, 1, maxTargets*maxDets*pwdDim)
function getPWD(opt, input, detections, t, detexlabels)
  ...
  for mb = 1,opt.mini_batch_size do
    -- max_n=2时，
    local mbStart = opt.max_n * (mb-1)+1 --1,3,5……
    local mbEnd =   opt.max_n * mb  --2,4,6
    -- max_m=4时，detection的某一序列的maxD个目标的temp_win个状态
    local mbStartD = opt.max_m * (mb-1)+1 --1,5,9
    local mbEndD =   opt.max_m * mb  -- 4,8,12

    -- det_x: (maxDets,pwdDim),detection的某一序列的maxD个目标的t+1时刻的状态
    local det_x =	detections[{{mbStartD, mbEndD},{t+1},{1,pwdDim}}]:clone():reshape(maxDets,pwdDim)

    -- inpPred: (maxTargets, pwdDim)
