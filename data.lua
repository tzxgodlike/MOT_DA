...
function getTracksAndDetsTables(seqTable, maxTargets, maxDets, cropFrames, correctDets,detfile)
  ...
  for _, seqName in pairs(seqTable) do
    ...
    --print('data:1306-getDataSize:  Ngt='..Ngt..',Fgt='..Fgt..',Dgt='..Dgt)
    ...
    if opt.real_dets ~= 0 then
      ...
      for t=1,Fgt do
        for i=1,Ngt do
          --cTr为detections的第t帧中距离真值tracks的第t帧第i个目标最近的检测的编号/下标
          local sDist, cTr = findClosestTrack(tracks[i][t], detections[{{},{t}}])
          --    print(i,t)
          --    print(sDist, cTr, thr)
          --    sleep(2)
      ...
      for i=1,N do labels[i] = i end--将第i行的F个元素全设为i
    end
...
-- zh
function prepareRealData( mode, sequences, trainingSequences, singleBatch )
  -- body
  ...
  local realData = opt.real_data == 1 --true
  local hamidData = false
  ...
  -- print('dets 1')
  -- printDim(DetsTab[1], 1)

  -- FLAG ALL DETECTIONS THAT REALLY EXIST- 标记所有检测真正存在
  for k,v in pairs(DetsTab) do  --v为(temp_win,D) 
  ...
  -- printDetEx(DetExTab[1])

  local maxd = torch.max(torch.sum(DetExTab[1],1))
  DetsTab[1] = DetsTab[1]:narrow(1,1,maxd)
  DetExTab[1] = DetExTab[1]:narrow(1,1,maxd)
  -- print('dets 1')
  -- printDim(DetsTab[1], 1)

  print('Normalizing data...',2)
...
function prepareData(mode, sequences, trainingSequences, singleBatch)
  ...
  if not realData and not hamidData then
    ...
    -- print('#trTracksTab:') print(#trTracksTab) print('trTracksTab[1]') print(trTracksTab[1]) abort()
    trackModel = learnTrajModel(trTracksTab)
    -- print('trackModel:') print(trackModel) abort()
    print('   ...done')
  end  
  ...
  if mode == 'real' then
    ...
    elseif hamidData then
      ...
      -- print('TracksTab') print(TracksTab) abort()
      print('   ...done')
    end
  end
  ...
  local dRange = getStateRange() --（3，D）
  ...
  -- print('LabTab') print(LabTab) abort()
  ...
  -- print('TracksTab') print(TracksTab)
  -- print('DetsTab') print(DetsTab)
  -- print('LabTab') print(LabTab) 
  -- print('ExTab') print(ExTab) 
  -- print('DetExTab') print(DetExTab) 
  -- abort()
  -- all done, return
  return TracksTab, DetsTab, LabTab, ExTab, DetExTab, SeqNames
...
function synthesizeDataY(trackModels, nSynth, mbSize, trSeqTable, trainingSet)
  ...
  for n=1,nSynth do
    ...
    for m=1,mbSize do
      ...
      local StateMaxX, StateMaxY = StateMax[seqName]['MaxX'],StateMax[seqName]['MaxX']
...
function getHamidData(nSamples, dataDir, opt)
  ...
  print('Checking for file validity...')
  ...
  print('...done! '..nFiles..' valid files found.')
...
function sampleTrajectoryHamid(m, stateDim, imH, imW)
  ...
  while not validTraj do
    ...
    for d = 1,stateDim do
      -- get linear sample
      featInd = 3+2*(d-1)--3,5,7,9 状态下表
      local sx = sampleNormal(m[1][featInd], m[2][featInd])

      featInd = 3 + 2*stateDim + (d-1)--11,12,13,14 速度下标
      local mv = sampleNormal(m[1][featInd], m[2][featInd])
      local ex = sx + mv * F
      fullVec[2*d-1][1] = sx
      fullVec[2*d][1] = mv      
    end
    --fullVec：[x,xv,y,yv,w,wv,h,hv]'
    -- print(fullVec)
    -- abort()
    ...
    for t=s+1,e do
      ...
      -- 匀速运动模型，fullVec= transMat x fullVec + eta
...
function normalizeData(dataTab, detTab, backwards, n, m, seqNamesTab, singleBatch)
  ...
  -- print('normImSize') print(normImSize) abort()
      ...
      if torch.sum(trueDet) > 1 then 			-- handle special case <= 1 det
        ...
        for d=1,stateDim do
          ...
          elseif normImSize then
            ...
            -- end
            -- 上面三行改如下0510改，之前只是简单的从文件中观测出x,y的最大值-------------
            if d%2 == 0 then divFactor = StateMax[seqName]['MaxY']
            else divFactor = StateMax[seqName]['MaxX']
            end
          end
          ...
          if dStd[d]<=0 then print(string.format('std. dev. not positive: %f', dStd[d]),3) end
        end
...
------------------------------------------
--- 逆 归一
function NormlizeBack(Tracks)
  local xpr = X_predict:clone()
  local meas = Meas:clone()
  local dMean = torch.zeros(stateDim)
  local dStd = torch.zeros(stateDim) 
  -- print(xpr) abort()
  local seqname = ''
  if TESTING then 
  ...
  for d=1,stateDim do
    if d%2 == 0 then 
      dStd[d] = maxmin_State[seqname]['MaxY']
      dMean[d] = dStd[d]/2
    else
      dStd[d] = maxmin_State[seqname]['MaxX']
      dMean[d] = dStd[d]/2
    end
  end
  ...
  for d=1,stateDim do
    --  print(vloc[{{},{},{d}}]:reshape(1,50))
    if dStd[d]>0 then
      xpr[{{},{d}}] = xpr[{{},{d}}] * dStd[d] + dMean[d]
      meas[{{},{d}}] = meas[{{},{d}}] * dStd[d] + dMean[d]
    end
  end
  -- print('X_predict') print(X_predict)
  -- print('Meas') print(Meas)
  -- abort()
